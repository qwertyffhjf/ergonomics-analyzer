...
# Получаем путь к видео из аргументов командной строки
if len(sys.argv) < 2:
...
# GitHub Models token берётся из секрета
GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN")
if not GITHUB_TOKEN:
    print("ERROR: GITHUB_TOKEN not set")
    sys.exit(1)

MODEL = "gpt-4o-mini"  # можно заменить на meta/llama-3.2-11b-vision-instruct
...
def analyze_frame(frame_bytes):
    """Отправляет кадр в GitHub Models и возвращает JSON"""
...
def process_video(video_path):
    ...
    frame_interval = max(1, int(fps))  # 1 кадр в секунду
    frame_count = 0
    prev_frame = None
    results = []

    print(f"Обработка видео: {video_path}")
    print(f"FPS исходного видео: {fps}, интервал: {frame_interval}")
